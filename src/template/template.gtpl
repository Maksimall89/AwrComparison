...
<p>Instance Efficiency Percentages: {{.NonParseCPU}}</p>
<p>Instance Efficiency Percentages: {{.ParseCPUElapsd}}</p>
<p>Instance Efficiency Percentages: {{.SoftParse}}</p>
<p>Shared Pool Statistics: {{.SharedPoolStatistics}}</p>

<h3>Самые долгие запросы Top Foreground Events By Total Wait Time</h3>
...
<p><b>Foreground Wait Class</b> и <b>Foreground Wait Events</b> показывают классы, которые провели в ожидании большего всего и список всех клиентов, которые также ожидали. Данный раздел является более подробных продолжение предыдущего и как провело его можно игнорировать если только вы не занимаетесь тонкой настройкой кластера т.к. например тоже ожидание <b> SQL*Net message from client</b> показывает время в рамках, которого клиент не обращался к базе данных с запросами.</p>
<p><b>Background Wait Events</b> опять же показывает время простоя событий, но теперь фоновыми процессами (LGWR, DBWR и т.д.).</p>
